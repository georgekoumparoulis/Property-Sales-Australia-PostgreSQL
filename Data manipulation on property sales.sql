...
/*
Top sales by year and postcode
*/
...
from(
	select 
		...
		pstcode, 
		total_sales, 
		row_number()over(partition by t_year order by t_year, total_sales desc) as rn
	from(
		select
			distinct(postcode) as pstcode,
			extract (year from datesold) as t_year,
			sum(price) over(partition by extract (year from datesold), postcode) as total_sales
		from
			raw_sales
		order by 
			t_year, 
